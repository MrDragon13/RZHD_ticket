...
| **Темы и выбор мест** | **РЖД** по умолчанию и **Неон** (переключатель после входа в терминал); тема **запоминается в браузере** (`localStorage`, ключ версии в коде — при обновлении ключа снова показывается РЖД). В режиме выбора мест кнопка **«Подтвердить»** закреплена **внизу правой колонки** под картой маршрута; светлая тема — выверенный контраст seat-picker (вкладки, купе, ячейки). |
...
- **две темы оформления** — корпоративная «РЖД» по умолчанию и «Неон» (`data-theme`), переключатель в шапке после входа в терминал рядом с языком; запоминание выбора в **`localStorage`** (ключ версии в `frontend/app.js`, при смене версии ключа новые посетители снова видят РЖД); для темы РЖД отдельно настроена палитра **выбора вагона и мест** (читаемые панели, акценты, купе и ячейки);
...
- `index.html` — разметка терминала; в `<head>` короткий скрипт выставляет `data-theme` из `localStorage` до загрузки CSS (ключ темы совпадает с `THEME_STORAGE_KEY` в `app.js`);
