...
«Путь» — олимпиадный **MVP-прототип**, в центре которого **большая языковая модель (LLM)** и сценарии **голосового и текстового ИИ-ассистента**: пассажир формулирует намерение естественной речью, система **понимает смысл**, ведёт **контекстный диалог**, объясняет рекомендации «человеческим» языком и сопровождает пользователя до **демонстрационного билета**. Это сознательный ответ на тренд **AI-first self-service**: не формы, а **разговор о поездке** плюс данные расписания РЖД и **гибридная логика** (правила + LLM). Интерфейс рассчитан на **сенсорный киоск**. Ниже — отдельный блок о том, как **интеллект и «умные» решения без нейросети** прошиты **на каждом шагу** сценария и в чём **принципиальное отличие** от «терминала с поиском».

Документ задаёт **единое описание** архитектуры, пользовательского потока и границ прототипа — для разработки, сопровождения кода и передачи контекста команде.
...
Здесь зафиксировано то, что не умещается в одной таблице эндпоинтов: **интеллект встроен в сценарий целиком**, а не прикручен отдельной кнопкой «спросить ИИ».

### ИИ «рядом» с пассажиром по всей дорожке

Пользователь **не собирает анкету** — он формулирует намерение голосом или текстом. На этом фундаменте выстроена цепочка вызовов и продуктовых решений:
...
- **AI Fact** на карте — нейтральный интересный факт о маршруте или городе, чтобы экран не был «пустой картой».
- **Сравнение двух поездов** — структурированное текстовое сравнение по данным РЖД (время, цена, комфорт, выводы), озвучка; при недоступности модели — **осмысленный fallback**, не заглушка «ошибка».
- **Чат «Помощь»** — ответы в стиле оператора киоска (`/api/support-chat`); тот же принцип: DeepSeek или связный fallback.
...
Итого по опыту пользователя: **на каждом шагу он либо говорит с интеллектуальным слоем, либо видит результат его работы** — объяснение, факт, сравнение, поддержка.

### Не нейросеть — но не «тупо работает»

Там, где нельзя или не нужно отдавать решение модели, сознательно стоят **прозрачные и проверяемые механики** — это не «отсутствие ума», а **инженерная дисциплина**:

- **Скоринг и фильтрация поездов** — детерминированные правила поверх данных РЖД; **воспроизводимая** логика приоритетов и прозрачная трассировка решений (удобно сопровождать и отлаживать в эксплуатации).
...
- **Политики разметки вагонов и мест**, учёт неполных ответов РЖД — описано в коде и [`docs/RZD_LAYOUT_POLICY.md`](docs/RZD_LAYOUT_POLICY.md).
- **Адаптивный киосковый UX** — fluid-типографика, темы **РЖД / Неон**, **prefers-reduced-motion**, **safe-area**, приостановка **idle-таймера** на время HTTP к API — чтобы интерфейс **подстраивался под человека и условия**, а не выкидывал сессию посреди ожидания ответа.
- **Демо-контуры доверия** — имитация входа по SMS, имитация оплаты перед билетом: не реальные платежи, но **сценарная правдоподобность**, которой нет у «голого списка рейсов».

В итоге это не «чат, прикрученный к справочнику», а **цельный киоск**: умный слой проходит через диалог, карту, рекомендации, сравнение, поддержку и голосовые сценарии, а детерминированная часть остаётся **предсказуемой, проверяемой и согласованной с данными РЖД** — контролируемость рядом с гибкостью языковой модели.
...
- **Доступность:** сочетание голоса и текста, live-области для объявлений ассистента, предсказуемые состояния ожидания (сфера, скелетоны списка поездов); на экране выбора языка — **демо «режим для слабовидящих»** (крупнее масштаб и контраст до входа в сценарий, сброс при возврате на язык).
...
Интерфейс рассчитан на **наглядную работу с экрана киоска**: крупные элементы, **две темы оформления** (корпоративная **РЖД** по умолчанию и футуристичный **Неон**), карта маршрута с разным акцентом в светлой и тёмной теме, AI-сфера, голосовой ввод и голосовые ответы. Отдельное внимание уделено **адаптивному отображению**: интерфейс сохраняет читаемость и композицию от компактного окна браузера до полноформатного киоска — это сознательное усиление по критерию «качество UX при разных условиях эксплуатации».
...
**Сопровождение описания:** при заметных изменениях пользовательского потока, интерфейса или **контрактов API** синхронизируйте этот документ с кодом — так README остаётся **проверяемым описанием** проекта для команды и интеграторов.
