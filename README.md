...
- `styles.css` — тема **РЖД** (светлая корпоративная) по умолчанию и альтернативная тёмная **Неон** на CSS-переменных; единая система токенов для карты, карточек поездов и **seat-picker**; **адаптивность** реализована на чистом CSS (`clamp`, `vmin`/`vw`/`vh`, при необходимости `dvh`, `env(safe-area-inset-*)`) без обязательной сборки под каждый класс устройств;
