...
### Фрагментированные данные РЖД (для разработчиков)

Если из РЖД приходит **не полный** состав мест или счётчиков по полкам, договорённость такая: **места вне явно переданного фрагмента считаем проданными** при отрисовке схемы; малые числа вместимости из ответа **не трактуем как «вагон на N мест»**, если это явный фрагмент списка. Подробно и с отсылками к коду: [`docs/DEV_RZD_FRAGMENT_RULES.md`](docs/DEV_RZD_FRAGMENT_RULES.md).
