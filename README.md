...
| **Темы и выбор мест** | Две цветовые схемы — **Неон** и корпоративная **РЖД** (переключатель в шапке); для светлой темы отдельно выверены **контраст, фон карточек и акценты** в сложном блоке **выбора вагона и места** (seat-picker): вкладки, купе, ячейки, сумма — без «залипших» неоновых цветов на белом. |
...
Интерфейс специально сделан презентационным и touch-first: крупные элементы, **две темы оформления** (футуристичный **Неон** и светлая **РЖД**), неоновая карта в тёмной теме, AI-сфера, голосовой ввод и голосовые ответы.
...
- **две темы оформления** — «Неон» и корпоративная «РЖД» (`data-theme`), переключатель рядом с языком; для темы РЖД отдельно настроена палитра **выбора вагона и мест** (читаемые панели, акценты, купе и ячейки);
...
- `styles.css` — тема **Неон** (тёмная) по умолчанию и альтернативная светлая **РЖД** на CSS-переменных; единая система токенов для карты, карточек поездов и **seat-picker**;
