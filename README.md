...
| **UX киоска** | Голос и текст: переключатель **«Текст»** в шапке рядом с языком, поле ввода — в колонке ассистента; карта маршрута; **липкая** верхняя зона панели (сфера, история, чипы); индикация ожидания; блокировка двойных действий; реплика пользователя не дублируется отдельной строкой — только **история диалога**; лёгкие **motion-переходы** между экраном языка и терминалом, при входе в оформление — анимация смены основной области и появления блока выбора мест; после показа демо-билета через **~60 с** — автоматический возврат на экран выбора языка с **полным сбросом** контекста; элементы **доступности** (`aria-live` и др.). Системное «меньше движения» (`prefers-reduced-motion`) отключает анимации. |
...
| **Проверяемость** | **Набор автотестов** на `pytest`: smoke API, контракт `/api/checkout-voice-intent`, **лимит POST** к `/api/*`, **контрактные проверки** ключевых защитных паттернов во `frontend/app.js` и разметке (без браузера); пошаговый сценарий демонстрации; блок **черновых UX-ориентиров** из пробных прогонов (ниже). |
...
- **Автоматические тесты:** расширенный контур на **`pytest`** — не только smoke API, но и изолированные проверки **anti-flood middleware**, контрактных ответов и **регрессионных строк** во frontend (отмена параллельных запросов диалога, лимит длины сообщения, защита от гонок при выборе поезда и повторном STT); см. раздел «Автотесты» ниже.
...
Так блок **дополняет** автотесты и демонстрационный сценарий «человеческим» измерением без смешения с официальной отчётностью.
...
Интерфейс специально сделан презентационным и touch-first: крупные элементы, **две темы оформления** (корпоративная **РЖД** по умолчанию и футуристичный **Неон**), карта маршрута с разным акцентом в светлой и тёмной теме, AI-сфера, голосовой ввод и голосовые ответы.
...
- по истечении **~60 с** после показа демо-билета — возврат на экран выбора языка и очистка диалога (сценарий «пустого» киоска);
...
6. Система показывает, как поняла запрос (карточки «Отправление» / «Прибытие» появляются только если задано осмысленное **временное окно**, а не заглушка «весь день»):
   - Москва → Казань;
   - дата 2026-05-06;
   - прибытие 07:00–09:00;
   - предпочтения: сон и комфорт.
7. На карте подсвечивается маршрут (акцент зависит от выбранной темы).
...
12. Если пользователь не трогает терминал, примерно через минуту интерфейс возвращается к **выбору языка**, контекст диалога очищается (удобно для демонстрации и публичного киоска).
...
- `styles.css` — тема **РЖД** (светлая корпоративная) по умолчанию и альтернативная тёмная **Неон** на CSS-переменных; единая система токенов для карты, карточек поездов и **seat-picker**;
...
**Сопровождение описания:** при заметных изменениях пользовательского потока или интерфейса стоит синхронизировать этот документ с кодом — так README остаётся проверяемой «витриной» проекта для презентаций и защиты.
...
| **Тестирование** | **pytest**: smoke API, middleware POST rate-limit, контракт checkout-intent, **статические проверки** frontend | Осознанный **баланс**: закрыть **критичные и средние** риски (API, злоупотребление POST, ключевые строки защиты в клиенте) без полного E2E в CI — для киоска это даёт максимум уверенности при минимальной стоимости поддержки. |
...
В каталоге `backend/tests/` собран **набор регрессии под киоск**: **smoke** (`GET /api/health`, заголовок `X-Request-Id`, контракт `/api/checkout-voice-intent`), изолированная проверка **лимита POST** (`PostApiRateLimitMiddleware`) и **контрактные тесты** на наличие защитных паттернов во `frontend/app.js` и `maxlength` у поля ввода в `index.html` — чтобы типичные поломки от «душнилы-тестировщика» (спам запросов, гонки кликов, переполнение текста) ловились **без браузера** и не возвращались в релиз незамеченными.
