...
«Путь» — олимпиадный **MVP-прототип** интеллектуального билетного терминала: автоматизированная касса нового поколения, где пассажир формулирует намерение **голосом или текстом**, система понимает смысл, подбирает поезда по **данным поиска РЖД**, объясняет выбор и ведёт по **линейному сценарию** до **демонстрационного билета**. Интерфейс рассчитан на **сенсорный киоск** и снижение когнитивной нагрузки по сравнению с многоэкранными формами.

Этот документ можно использовать **как основу презентации и ответов жюри**: ниже — формулировки целей, аудитории, пользовательского потока и измеримости без подмены границ прототипа.
...
| **Ценность для инфраструктуры** | Модель самообслуживания с ИИ-ассистентом снижает нагрузку на справочные и кассовые линии за счёт **понятного первого шага** («куда и когда») и объяснений к вариантам. |
| **Интеллект** | **Гибрид**: детерминированный скоринг и данные расписания **+** языковая модель для разбора фраз, объяснений и фактов; при сбое внешнего API — **устойчивый fallback**, сценарий не обрывается. |
| **Данные поездов** | Адаптер к публичному контуру поиска ( **aiorzd** / live и demo-режим); политика обработки неполных ответов РЖД задокументирована: [`docs/RZD_LAYOUT_POLICY.md`](docs/RZD_LAYOUT_POLICY.md). |
| **UX киоска** | Крупные зоны касания, **голос и текст**, визуальная карта маршрута, индикация ожидания, блокировка двойных действий; элементы **доступности** (`aria-live` и др.). |
| **Безопасность** | Ключ LLM только на **сервере**; секреты не попадают в статический frontend. |
| **Наблюдаемость** | Заголовок **`X-Request-Id`** на запросах — связка клиент ↔ логи backend для разбора инцидентов и задержек. |
| **Развёртывание** | **Docker Compose**, воспроизводимый запуск; вариант с **Caddy** и HTTPS для работы голосового ввода в браузере. |
| **Проверяемость** | Автоматические **smoke-тесты** API и воспроизводимый **сценарий защиты** (ниже). |
...
- сквозной сценарий от выбора языка до **демонстрационного билета**;
- разбор **естественной речи** и уточняющего диалога (в т.ч. с передачей **контекста** последних реплик в запрос к модели);
- поиск поездов в **live** или **demo**-режиме, рекомендации и пояснения;
...
## Рекомендации: прозрачная «магия»

Система **не является «чёрным ящиком»**:

1. **Детерминированный скоринг** учитывает время в пути, ценовые сигналы (по данным ответа РЖД), наличие мест, попадание в окна отправления/прибытия, предпочтения (сон, цена и т.д.).
2. **Языковая модель** усиливает **понимание запроса** и генерирует **короткое голосовое объяснение** рекомендуемого варианта; при нестандартных формулировках возможно **LLM-ранжирование** порядка поездов.
3. Итог: рекомендация **объяснима** и привязана к данным расписания, а не к случайному ответу модели.

---

## Ориентиры KPI для пилотного этапа (не текущие замеры)
...
Так таблица показывает жюри **зрелость продуктового мышления**, не подменяя собой фактические данные прототипа.
...
1. **MVP / олимпиада** — текущий репозиторий: полный демо-контур, live/demo РЖД, гибридные рекомендации.
