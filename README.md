...
- после выбора языка — **демо-вход по телефону и SMS** (имитация, без хранения персональных данных на сервере); условные ФИО и документ отображаются на демо-билете; при **отсутствии действий ~60 с** — предупреждение за **5 с** и возврат на язык с очисткой диалога (сценарий публичного киоска);
...
10. Терминал показывает варианты поездов и рекомендации.
11. Пользователь касается карточки поезда.
12. Нажимает «Оформить демо-билет».
13. Система имитирует оформление и показывает демо-билет с QR-зоной.
14. Если пользователь не трогает терминал **около минуты**, появляется предупреждение и интерфейс возвращается к **выбору языка**, контекст диалога очищается (удобно для демонстрации и публичного киоска).
