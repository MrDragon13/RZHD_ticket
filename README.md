...
Ответы языковой модели сейчас приходят **целиком по REST** (диалог, пояснения, факты). **Потоковая выдача текста (SSE / по-токенный вывод)** сознательно **отложена**: её можно добавить позже, когда стабилизируются сценарии и метрики задержки; это не требуется для работы текущего MVP и fallback.
