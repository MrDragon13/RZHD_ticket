/**
 * Клиент терминала «Путь»: без сборщика; словари UI — `./app-i18n.js`. HTTP через `./api-client.js`
 ...
 */
// HTTP: fetchApi в ./api-client.js; словари UI в ./app-i18n.js; порядок загрузки — index.html.
...
/** Ключ темы в localStorage; задаётся в `<head>` как `window.PATH_THEME_STORAGE_KEY`. */
const THEME_STORAGE_KEY =
  typeof window !== "undefined" && window.PATH_THEME_STORAGE_KEY
    ? window.PATH_THEME_STORAGE_KEY
    : "path-theme-v3-default-rzd";
 ...
 * @param {{ animate?: boolean; duration?: number; easing?: 'smoothstep' | 'cubicOut' }} [options]
 */
...
function applyRouteGeometry(visual, labelOverride) {
  ...
  if (routeReduced || hasClip) {
    syncDecorativeRouteLines(visual.line, { animate: false });
...
// --- Старт приложения: подписи по текущему языку, тема из localStorage (`PATH_THEME_STORAGE_KEY` в <head>), модалки ---
updateTextInputToggleLabels();
applySupportChatChrome();
applyCheckoutLoadingTexts();
applyCompareChrome();
...
/** Тема: THEME_STORAGE_KEY из window.PATH_THEME_STORAGE_KEY (<head>); дефолт rzd; раннее применение темы — там же до paint. */
